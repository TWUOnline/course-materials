(function () {
  ...
  function update_total_correct() {
    ...
    for (var i = 0; i < t.length; i++) {
      ...
      // ---- Mindset totals (small text, same line as points) ----
      var FIXED = [1, 4, 6, 7, 8, 11];
      var GROWTH = [2, 3, 5, 9, 10, 12];

      function sumByQuestions(list) {
        var total = 0;
        list.forEach(function (qNum) {
          var idx = qNum - 1;
          var info = groupInfo[idx];
          if (!info) return;
          total += info.earned;
        });
        return total;
      ...
      var fixedTotal = sumByQuestions(FIXED);
      var growthTotal = sumByQuestions(GROWTH);
      ...
      if (!section.classList.contains("no-points")) {
        t[i].textContent =
          ...
          growthTotal +
          " • My Dominant mindset: " +
          (fixedTotal <= growthTotal ? fixedTotal : growthTotal);
      } else {
  ...
  function solveme_func() {
    ...
    if (tolAttr && Number(tolAttr) > 0) {
      ...
      var anyMatch = real_answers
        ...
        .filter(function (x) {
          return !Number.isNaN(x) && !Number.isNaN(myNum);
        })
        .some(function (x) {
          return Math.abs(x - myNum) < tol;
        });
      if (anyMatch) cl.add("webex-correct");
  ...
  window.onload = function () {
    ...
    for (var k = 0; k < solveme.length; k++) {
      ...
      var parsed = [];
      try {
        parsed = JSON.parse(solveme[k].dataset.answer || "[]");
      } catch (_) {
        parsed = [];
      }
      if (cl.contains("ignorecase"))
        parsed = parsed.map(function (a) {
